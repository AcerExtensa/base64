...
enc_reshuffle (__m128i in)
{
	...
	// indices_cd = [00000000|00000000|0000cccc|ccdddddd]
	const __m128i indices_cd = _mm_and_si128(in, _mm_set1_epi32(0x00000FFF));

	// indices_ab = [0000aaaa|aabbbbbb|00000000|00000000]
	const __m128i indices_ab = _mm_and_si128(_mm_slli_epi32(in, 4), _mm_set1_epi32(0x0FFF0000));

	// merged     = [0000aaaa|aabbbbbb|0000cccc|ccdddddd]
	const __m128i merged = _mm_or_si128(indices_ab, indices_cd);

	// indices_bd = [00000000|00bbbbbb|00000000|00dddddd]
	const __m128i indices_bd = _mm_and_si128(merged, _mm_set1_epi32(0x003F003F));

	// indices_ac = [00aaaaaa|00000000|00cccccc|00000000]
	const __m128i indices_ac = _mm_and_si128(_mm_slli_epi32(merged, 2), _mm_set1_epi32(0x3F003F00));

	// indices    = [00aaaaaa|00bbbbbb|00cccccc|00dddddd]
	const __m128i indices = _mm_or_si128(indices_ac, indices_bd);
